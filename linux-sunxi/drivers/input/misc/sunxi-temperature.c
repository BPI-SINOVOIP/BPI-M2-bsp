...
#else
#ifdef CONFIG_PM
struct dev_pm_domain temp_pm_domain;
#endif
#endif
};
...
static ssize_t sunxi_temp_show_temp(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	printk("%s: enter \n", __func__);
	return sprintf(buf, "temperature:%lld\n", tempetature);
}
...
static DEVICE_ATTR(temperature, S_IRUGO|S_IWUSR|S_IWGRP|S_IROTH,
		sunxi_temp_show_temp, NULL);
...
static void sunxi_temp_read(uint64_t *data)
{
	...
	if (1000 < history_record[circle_num])
		circle_num++;
...
static void sunxi_temp_reg_init(void)
{
	...
	dprintk(DEBUG_INIT, "AW_VIR_TP_BASE + 0x00 = 0x%x\n", readl(temp_data->base_addr + 0x00));
	dprintk(DEBUG_INIT, "AW_VIR_TP_BASE + 0x04 = 0x%x\n", readl(temp_data->base_addr + 0x04));
	dprintk(DEBUG_INIT, "AW_VIR_TP_BASE + 0x0c = 0x%x\n", readl(temp_data->base_addr + 0x0c));
	dprintk(DEBUG_INIT, "AW_VIR_TP_BASE + 0x10 = 0x%x\n", readl(temp_data->base_addr + 0x10));
	dprintk(DEBUG_INIT, "AW_VIR_TP_BASE + 0x18 = 0x%x\n", readl(temp_data->base_addr + 0x18));
}

#ifdef CONFIG_HAS_EARLYSUSPEND
static void sunxi_temp_early_suspend(struct early_suspend *h)
{
	struct sunxi_temp_data *data =
		container_of(h, struct sunxi_temp_data, early_suspend);
...
static void sunxi_temp_late_resume(struct early_suspend *h)
{
	struct sunxi_temp_data *data =
		container_of(h, struct sunxi_temp_data, early_suspend);
...
#else
#ifdef CONFIG_PM
static int sunxi_temp_resume(struct device *dev)
{
	dprintk(DEBUG_SUSPEND, "bma250: resume\n");
	
	if (NORMAL_STANDBY == standby_type) {
		mutex_lock(&temp_data->enable_mutex);
		if (atomic_read(&temp_data->enable)==1) {
			schedule_delayed_work(&temp_data->work,
				msecs_to_jiffies(atomic_read(&temp_data->delay)));
		}
		mutex_unlock(&temp_data->enable_mutex);
...
static int sunxi_temp_suspend(struct device *dev)
{
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	temp_data->early_suspend.suspend = sunxi_temp_early_suspend;
	temp_data->early_suspend.resume = sunxi_temp_late_resume;
	register_early_suspend(&temp_data->early_suspend);
#else
#ifdef CONFIG_PM
	temp_data->temp_pm_domain.ops.suspend = sunxi_temp_suspend;
	temp_data->temp_pm_domain.ops.resume = sunxi_temp_resume;
	temp_data->temp_dev->dev.pm_domain = &temp_data->temp_pm_domain;
#endif
#endif
...
{
#ifdef CONFIG_HAS_EARLYSUSPEND
	unregister_early_suspend(&temp_data->early_suspend);
#endif
	sysfs_remove_group(&temp_data->temp_dev->dev.kobj, &sunxi_temp_attribute_group);
	input_unregister_device(temp_data->temp_dev);
	kfree(temp_data);
