...
static void update_timer_locked(struct alarm_queue *base, bool head_removed)
{
	...
	bool is_wakeup = base == &alarms[ANDROID_ALARM_RTC_WAKEUP] ||
			...
			base == &alarms[ANDROID_ALARM_RTC_SUSPEND_WAKEUP];
...
int alarm_set_rtc(struct timespec new_time)
{
	...
	alarms[ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP].delta =
        ...
        alarms[ANDROID_ALARM_RTC_SHUTDOWN_WAKEUP].delta =
        alarms[ANDROID_ALARM_RTC_SUSPEND_WAKEUP].delta =
...
static int alarm_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	hrtimer_cancel(&alarms[ANDROID_ALARM_RTC_SUSPEND_WAKEUP].timer);
	...
	tmp_queue = &alarms[ANDROID_ALARM_RTC_SUSPEND_WAKEUP];
	if (tmp_queue->first&& (!wakeup_queue ||
		hrtimer_get_expires(&tmp_queue->timer).tv64 <
		hrtimer_get_expires(&wakeup_queue->timer).tv64)) {
		wakeup_queue = tmp_queue;
	...
	if (wakeup_queue) {
		...
		if (rtc_current_time + 1 >= rtc_alarm_time) {
			...
			update_timer_locked(&alarms[
				ANDROID_ALARM_RTC_SUSPEND_WAKEUP], false);
			err = -EBUSY;
			spin_unlock_irqrestore(&alarm_slock, flags);
		}
	}
...
static int alarm_resume(struct platform_device *pdev)
{
	...
	update_timer_locked(&alarms[ANDROID_ALARM_RTC_SHUTDOWN_WAKEUP],
									false);
	update_timer_locked(&alarms[ANDROID_ALARM_RTC_SUSPEND_WAKEUP],
									false);
	spin_unlock_irqrestore(&alarm_slock, flags);
