...
static int btusb_resume(struct usb_interface *intf)
{
	...
	if (test_bit(BTUSB_INTR_RUNNING, &data->flags)) {
		...
		RTKBT_DBG("btusb_resume, BTUSB_INTR_RUNNING true, submit intr urb ok.");
	}
	...
	if (test_bit(BTUSB_BULK_RUNNING, &data->flags)) {
		...
		RTKBT_DBG("btusb_resume, BTUSB_BULK_RUNNING true, submit bulk urb ok.");
	}

	if (test_bit(BTUSB_ISOC_RUNNING, &data->flags)) {
		if (btusb_submit_isoc_urb(hdev, GFP_NOIO) < 0) {
			clear_bit(BTUSB_ISOC_RUNNING, &data->flags);
			RTKBT_DBG("btusb_resume, BTUSB_ISOC_RUNNING true, submit isoc urb ok.");
		} else {
			btusb_submit_isoc_urb(hdev, GFP_NOIO);
			RTKBT_DBG("btusb_resume, BTUSB_BULK_RUNNING true, submit isoc urb fail.");
		}
	}
	...
	RTKBT_DBG("btusb_resume, BTUSB_SUSPENDING clear, ok.");
	spin_unlock_irq(&data->txlock);
	schedule_work(&data->work);
	...
	switch (pm_event)
	{
		...
		case PM_HIBERNATION_PREPARE:
			...
			if (!device_may_wakeup(&udev->dev))
			{
				...
				RTKBT_DBG("remote wakeup not support, not set\n");
			}
			break;
