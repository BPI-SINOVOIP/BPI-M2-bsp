...
#define hdr_ratio 4//0//0/2/3/4/8/16
...
0x0206,0x00,//COAR_INTEGR_TIM2[15:8] ;
0x0207,0x10,//COAR_INTEGR_TIM2[7:0] ;
0x0208,0x00,//ANA_GA_CODE_GL[15:8] ;0x0000
//0x0209,0x45,
0x0209,0x86,//ANA_GA_CODE_GL[7:0] ;
#if 0
0x020E,0x01,//-/-/-/-/-/-/AWB_GAIN_GREEN[9:8] ;
0x020F,0xb7,//AWB_GAIN_GREEN[7:0] ;
0x0210,0x01,//-/-/-/-/-/-/AWB_GAIN_RED[9:8] ;
0x0211,0xb7,//AWB_GAIN_RED[7:0] ;
0x0212,0x01,//-/-/-/-/-/-/AWB_GAIN_BLUE[9:8] ;
0x0213,0xb7,//AWB_GAIN_BLUE[7:0] ;
#endif
0x020E,0x01,//-/-/-/-/-/-/AWB_GAIN_GREEN[9:8] ;
0x020F,0x00,//AWB_GAIN_GREEN[7:0] ;
0x0210,0x01,//-/-/-/-/-/-/AWB_GAIN_RED[9:8] ;
0x0211,0x00,//AWB_GAIN_RED[7:0] ;
0x0212,0x01,//-/-/-/-/-/-/AWB_GAIN_BLUE[9:8] ;
0x0213,0x00,//AWB_GAIN_BLUE[7:0] ;
...
0x3276,0x01,//-/-/-/-/-/-/-/PWBSW ;	//20130814
0x3277,0x1a,//PWBGAINR[7:0] ;
0x3278,0x00,//PWBGAINGR[7:0] ;
0x3279,0x00,//PWBGAINGB[7:0] ;
0x327A,0x09,//PWBGAINB[7:0] ;
...
static struct regval_list sensor_quxga_regs[] = {// FPS = 30fps   Size = 3280x2464
//stream off
0x0100,0x00,//-/-/-/-/-/-/-/MODE_SELECT ;
...
0x0408,0x00,//-/-/-/DCROP_XOFS[12:8] ;
0x0409,0x00,//64,//DCROP_XOFS[7:0] ;
0x040A,0x00,//-/-/-/-/DCROP_YOFS[11:8] ;
0x040B,0x00,//64,//DCROP_YOFS[7:0] ;
0x040C,0x0c,//B,//-/-/-/DCROP_WIDTH[12:8] ;
0x040D,0xd0,//B8,//DCROP_WIDTH[7:0] ;
0x040E,0x09,//8,//-/-/-/-/DCROP_HIGT[11:8] ;
0x040F,0xa0,//98,//DCROP_HIGT[7:0] ;
//BLC
0x3250,0x00,
0x3251,0xFF,//AREASEL[7:0] ;
0x3253,0x40,
...
0x0301,0x00,//-/-/-/-/VT_PIX_CLK_DIV[3:0] ;
0x0303,0x04,//-/-/-/-/VT_SYS_CLK_DIV[3:0] ;
0x0305,0x02,//-/-/-/-/-/PRE_PLL_CLK_DIV[2:0] ;
0x0306,0x00,//-/-/-/-/-/-/-/PLL_MULTIPLIER[8] ;
0x0307,0x41,//PLL_MULTIPLIER[7:0]  0x41:780Mbps 0x35:636Mbps 0x53:996Mbps 0x5b:1092Mbps
...
0x3030,0x00, //-/-/-/-/-/H_CROP[2:0] ; //sensor out width = 2584
0x321C,0x00, //-/-/-/-/-/-/-/DCROP_SEL ;
0x30A6,0x0B, //PLL_SNR_CNTL[1:0]/PLL_SYS_CNTL[1:0]/PLL_SINGLE_CKSP/PLL_SINGLE_SW/;
//0x0104,0x00, //-/-/-/-/-/-/-/GROUP_PARA_HOLD ;
0x0800,0x80, //TCLK_POST[7:3]/-/-/- ;
...
0x0104,0x01,//-/-/-/-/-/-/-/GROUP_PARA_HOLD ;
0x0301,0x03,//-/-/-/-/VT_PIX_CLK_DIV[3:0] ;
0x0303,0x02,//-/-/-/-/VT_SYS_CLK_DIV[3:0] ;
0x0305,0x03,//-/-/-/-/-/PRE_PLL_CLK_DIV[2:0] ;
0x0306,0x00,//-/-/-/-/-/-/-/PLL_MULTIPLIER[8] ;
0x0307,0x82,//PLL_MULTIPLIER[7:0] ;
0x0309,0x08,//-/-/-/-/OP_PIX_CLK_DIV[3:0] ;
...
0x0341,0xd6, //FR_LENGTH_LINES[7:0] ;
...
0x3251,0x3F,//AREASEL[7:0] ;
0x3253,0x40,//-/OB_BLACK[6:0] ;
//0x3253,0x00,

0x325F,0x05,//-/-/-/-/-/CALCSEL[2:0] ;
...
0x034A,0x08,//Y_ADDR_END[15:8] ;
0x034B,0x68,//Y_ADDR_END[7:0] ;
0x034C,0x05,//X_OUTPUT_SIZE[15:8] ;
0x034D,0x10,//X_OUTPUT_SIZE[7:0] ;
0x034E,0x02,//Y_OUTPUT_SIZE[15:8] ;
0x034F,0xE0,//Y_OUTPUT_SIZE[7:0] ;
0x0350,0x00,//CONT_FRCNT[7:0] ;
0x0381,0x01,//-/-/-/-/-/-/H_MONI[1:0] ;
0x0382,0x01,//-/-/-/-/-/-/V_MONI[1:0] ;
0x0383,0x00,//-/-/-/-/-/-/BINNING_WEIGHTING[1:0] ;
0x0401,0x02,//-/-/-/-/-/-/SCALING_MODE[1:0] ;
0x0405,0x14,//SCALE_M[7:0] ;
0x0408,0x00,//-/-/-/DCROP_XOFS[12:8] ;
0x0409,0x0A,//DCROP_XOFS[7:0] ;
0x040A,0x00,//-/-/-/-/DCROP_YOFS[11:8] ;
0x040B,0x00,//DCROP_YOFS[7:0] ;
0x040C,0x06,//-/-/-/DCROP_WIDTH[12:8] ;
0x040D,0x54,//DCROP_WIDTH[7:0] ;
0x040E,0x03,//-/-/-/-/DCROP_HIGT[11:8] ;
...
static struct regval_list sensor_720p_crop_regs[] = { //720: 1280*720@30fps //MHz pclk
// Extclk= 24Mhz  SPCK = 260Mhz   MIPI CLK = 780MHz
// FPS = 60.15   Size = 1296x736
//stream off
0x0100,0x00,//-/-/-/-/-/-/-/MODE_SELECT;
...
0x0306,0x00, //-/-/-/-/-/-/-/PLL_MULTIPLIER[8] ;
0x0307,0x48, //PLL_MULTIPLIER[7:0] ;
0x0309,0x08, //-/-/-/-/OP_PIX_CLK_DIV[3:0] ;
...
0x30AF,0x41, //PLL_MULT_ST[7:0] ;
0x30B0,0x01, //-/-/-/-/-/PRE_PLL_CNTL_ST[2:0] ;
0x0340,0x03, //FR_LENGTH_LINES[15:8] ; //vts = 1235    60fps: hts * vts = 4333333
0x0341,0x20, //FR_LENGTH_LINES[7:0] ;
0x0342,0x0b, //LINE_LENGTH_PCK[15:8] ; //hts = 3500
0x0343,0xb8, //LINE_LENGTH_PCK[7:0] ;
0x0346,0x03, //Y_ADDR_START[15:8] ; //sensor out height = 1156
0x0347,0x44, //Y_ADDR_START[7:0] ;
0x034A,0x06, //Y_ADDR_END[15:8] ;
0x034B,0x40, //Y_ADDR_END[7:0] ;
0x034C,0x05, //X_OUTPUT_SIZE[15:8] ;
0x034D,0x10, //X_OUTPUT_SIZE[7:0] ;
0x034E,0x02, //Y_OUTPUT_SIZE[15:8] ;
0x034F,0xe0, //Y_OUTPUT_SIZE[7:0] ;
...
0x0405,0x10, //SCALE_M[7:0] ;
0x0408,0x00, //-/-/-/DCROP_XOFS[12:8] ;
0x0409,0x05, //DCROP_XOFS[7:0] ;
0x040A,0x00, //-/-/-/-/DCROP_YOFS[11:8] ;
0x040B,0x03, //DCROP_YOFS[7:0] ;
0x040C,0x07, //-/-/-/DCROP_WIDTH[12:8] ;
0x040D,0x90, //DCROP_WIDTH[7:0] ;
0x040E,0x04, //-/-/-/-/DCROP_HIGT[11:8] ;
0x040F,0x48, //DCROP_HIGT[7:0] ;
...
static struct regval_list hdr_on_regs[] = {
////stream off
//0x0100,0x00,//-/-/-/-/-/-/-/MODE_SELECT;
0x0104,0x01,//-/-/-/-/-/-/-/GROUP_PARA_HOLD ;

0x3260,0x00,//-/-/-/-/-/-/GDANSASW/PEDAJSW ;
0x3280,0x00,//-/-/-/-/-/-/-/LSSCSW disable;
0x32BF,0x00,//0C,//-/-/-/-/WBPSW/BBPSW/ANRSW/ABPCSW ; BPC/ANR disable
//HM mode
0x32D0,0x03,//3,//-/-/-/-/-/-/WDRSW_VREG/WDRSW ;
//ES settings
#if hdr_ratio == 2
0x32D1,0x00,//WDR_MDSEL/-/-/-/-/-/WDRMD[1:0] ;   X2
#endif
#if hdr_ratio == 3
0x32D1,0x81,//WDR_MDSEL/-/-/-/-/-/WDRMD[1:0] ;   X3
#endif
#if hdr_ratio == 4
0x32D1,0x01,//WDR_MDSEL/-/-/-/-/-/WDRMD[1:0] ;   X4
#endif
#if hdr_ratio == 8
0x32D1,0x02,//WDR_MDSEL/-/-/-/-/-/WDRMD[1:0] ;   X8
#endif
#if hdr_ratio == 16
0x32D1,0x03,//WDR_MDSEL/-/-/-/-/-/WDRMD[1:0] ;   X16
#endif

0x32D2,0xFF,//WDRLV0[7:0] ;
...
static struct regval_list hdr_off_regs[] = {
////stream off
//0x0100,0x00,//-/-/-/-/-/-/-/MODE_SELECT;
0x0104,0x01,//-/-/-/-/-/-/-/GROUP_PARA_HOLD ;
...
0x3260,0x00,//-/-/-/-/-/-/GDANSASW/PEDAJSW ;
0x3280,0x00,//-/-/-/-/-/-/-/LSSCSW disable;
0x32BF,0x0C,//-/-/-/-/WBPSW/BBPSW/ANRSW/ABPCSW ; BPC/ANR disable
//HM mode
0x32D0,0x03,//-/-/-/-/-/-/WDRSW_VREG/WDRSW ;
//ES settings
#if hdr_ratio == 2
0x32D1,0x00,//WDR_MDSEL/-/-/-/-/-/WDRMD[1:0] ;   X2
#endif
...
//static int cal_tline(struct v4l2_subdev *sd)
//{
//  unsigne int ret_tline;
//	struct sensor_info *info = to_state(sd);
//  ret_tline=info->current_wins.hts*info->current_wins.vts/100;
//  
//}
...
static int sensor_s_exp(struct v4l2_subdev *sd, unsigned int exp_val)
{
	struct sensor_info *info = to_state(sd);
	unsigned char explow,exphigh;
	unsigned char short_explow,short_exphigh;
	unsigned int exp_tmp;
	unsigned int tline;//Q4 in us
	unsigned int band50,band60,band;//Q4
	unsigned int tmp,tmp_low = 0;
  ...
  /*tline=16*info->current_wins->hts*1000/(info->current_wins->pclk/1000);
	band50=16*(info->current_wins->pclk/info->current_wins->hts+50)/100;
	band60=16*(info->current_wins->pclk/info->current_wins->hts+60)/120;
	vfe_dev_dbg("width = %d, height = %d\n",info->current_wins->width, info->current_wins->height);
	vfe_dev_dbg("hts = %d, vts = %d\n", info->current_wins->hts, info->current_wins->vts);
	vfe_dev_dbg("tline = %d, tline*47/100 = %d\n", tline, tline*47/100);
	vfe_dev_dbg("band50 = %d, band60 = %d\n", band50, band60);*/
	
//	vfe_dev_dbg("exp_tmp = %d\n",exp_tmp);
  exphigh = (unsigned char) ( (0xff00&exp_tmp)>>8);
  explow  = (unsigned char) ( (0x00ff&exp_tmp) );
	
	//for normal and long exp
  printk("sensor_exp_real= %d\n", exp_tmp);
    sensor_write(sd, 0x0202, exphigh);	
    sensor_write(sd, 0x0203, explow);
  	sensor_read(sd,0x0203,&tmp_low);
	sensor_read(sd,0x0202,&tmp_high);
	tmp =((tmp_high<<8)& 0xff00) | (0x00ff&tmp_low);
	printk("sensor_set_exp_real= %d\n", tmp);
  #if 1
	if(hdr_ratio>=2)
	{//for short exp
	  unsigned int short_exp_tmp;
	  unsigned int short_exp_val = exp_val/(hdr_ratio);//Q4
	  short_exp_tmp = (short_exp_val+8)>>4;
//	  if(short_exp_val>=band50)
//	  {
//  	  short_exp_tmp = (short_exp_val+8)>>4;
//	    vfe_dev_dbg("sensor_set_short_exp_tmp = %d\n", short_exp_tmp);
//	  }
//	  else
//	  {
//	    short_exp_tmp = (band50+8)/16;
//	    vfe_dev_dbg("sensor_set_short_exp_tmp band limit= %d\n", short_exp_tmp);
//	  }
	  
    short_exphigh = (unsigned char) ( (0xff00&(short_exp_tmp))>>8);
    short_explow  = (unsigned char) ( (0x00ff&(short_exp_tmp)) );
	  
	  sensor_write(sd, 0x0206, short_exphigh);
	  sensor_write(sd, 0x0207, short_explow);
	}
  #endif
	info->exp = exp_tmp;
	return 0;
...
static int sensor_g_gain(struct v4l2_subdev *sd, __s32 *value)
{
	...
	//gain_tmp = ( ((gainhigh<<8) + gainlow)<<4 ) / 69;
	gain_tmp = ( ((gainhigh<<8) + gainlow)<<4 ) / 134;
	printk("sonsor read and get gain = %p",value);
...
//	gain_val = 16;
	...
	gain_tmp=((gain_val)*134)>>4;
	//gain_tmp = ((gain_val)*134)/ 10;
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
  ...
  sensor_read(sd,0x3253,&temp);
  printk("t4k05's 0x3253 value is 0x%x\n",temp);
...
static struct sensor_win_size sensor_win_sizes[] = {
      ...
      .hoffset    = (3280-QUXGA_WIDTH)/2,
      .voffset    = (2464-QUXGA_HEIGHT)/2,
      .hts        = 3454,
      .vts        = 2510,
      .pclk       = 130*1000*1000,
      .mipi_bps	  = 520*1000*1000,
      .fps_fixed  = 1,
      .bin_factor = 1,
      .intg_min   = 1<<4,
      .intg_max   = 2510<<4,
      ...
      .hoffset    = (1936-HD1080_WIDTH)/2,
      .voffset    = (1096-HD1080_HEIGHT)/2,
	  .hts		  = 3454,
	  .vts		  = 2510,
      .pclk       = 260*1000*1000,
      .mipi_bps	  = 520*1000*1000,
      ...
      .gain_min   = 1<<4,
      .gain_max   = 8<<4,
      .regs       = sensor_1080p_crop_regs,//sensor_1080p_regs //sensor_1080p_crop_regs
      .regs_size  = ARRAY_SIZE(sensor_1080p_crop_regs),//sensor_1080p_crop_regs
      .set_size   = NULL,
    },
#endif
	  /* sxga: 1280*960 */
	  ...
	  .width	  = SXGA_WIDTH,
	  .height	  = SXGA_HEIGHT,
	  .hoffset	  = (1640 - SXGA_WIDTH)>>1,
	  .voffset	  = (1232 - SXGA_HEIGHT)>>1,
	  .hts		  = 3500,
	  .vts		  = 1235,
	  .pclk 	  = 130*1000*1000,
	  .mipi_bps   = 520*1000*1000,
	...
	/* 720p: 1280*720 */
	{
      .width      = HD720_WIDTH,
      .height 	  = HD720_HEIGHT,
      .hoffset	  = (1296 - HD720_WIDTH)>>1,
      .voffset    = (736 - HD720_HEIGHT)>>1,
      .hts		  = 3500,
      .vts		  = 1235,
      .pclk		  = 130*1000*1000,
      .mipi_bps   = 520*1000*1000,
      ...
      .gain_min	  = 1<<4,
      .gain_max	  = 8<<4,
      .regs		  = sensor_720p_regs,//sensor_720p_regs//sensor_720p_crop_regs
...
//  printk("wsize->regs=0x%x\n",wsize->regs);
  ...
  if(hdr_ratio!=0)
    LOG_ERR_RET(sensor_write_array(sd, hdr_on_regs, ARRAY_SIZE(hdr_on_regs)))
  ...
  sensor_read(sd,0x3253,&val_3253);
  
  sensor_read(sd,0x3280,&val_3280);
  sensor_read(sd,0x3250,&val_3250);
  
  
  printk("t4k05######$$$$$$$$$$$$$$   black level is 0x%x\n",val_3253);
  
  printk("t4k05 0x3280 value is 0x%x\n",val_3280);
  printk("0x3250 value is 0x%x\n",val_3250);
