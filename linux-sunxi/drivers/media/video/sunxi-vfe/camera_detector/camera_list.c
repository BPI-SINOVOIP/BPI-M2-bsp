...
static void camera_pwr_on_hi257(__u32 list_index, __camera_info_t *camera_info)
{
    //power on reset
	camera_gpio_set_status(camera_info->stby_pin,1);//set the gpio to output
	camera_gpio_set_status(camera_info->reset_pin,1);//set the gpio to output
...
static void camera_pwr_off_hi257(__u32 list_index, __camera_info_t *camera_info)
{
    //standby and reset io
	camera_gpio_write(camera_info->stby_pin, camera_list[list_index].CSI_STBY_ON);
	msleep(100);
...
static __s32 camera_detect_hi257(__u32 list_index, struct i2c_adapter *i2c_adap)
{
    int ret;
	struct regval_list regs;
    __u32 i2c_addr  = camera_list[list_index].i2c_addr;
    ...
    list_print("try to detect hi257 ... \n");
	...
	if(regs.value[0] != 0xc4)
		return -ENODEV;
    
    list_print("detect hi257 success!!\n");
...
__camera_list_t camera_list[MAX_CAMERA_LIST_ITEM] = {
    ...
    CAMERA_LIST_ITEM_INIT(hi257,         1, 1, 0x40, 1, 0, 0, 1, 1, 0),
    ...
    CAMERA_LIST_ITEM_INIT(gc2235,        1, 1, 0x78, 1, 0, 0, 1, 1, 0),
};
