...
*/
static int subdev_i2c_tx(struct i2c_client * client, unsigned short reg, unsigned char val)
{
  struct i2c_msg msg;
  ...
  if (ret >= 0) {
    ...
    act_dev_dbg("subdev_i2c_tx 0x%4x = 0x%4x err!\n",
...
static int subdev_i2c_write(struct actuator_ctrl_t *act_ctrl, unsigned short halfword, void* pt)
{
  ...
  ret|=subdev_i2c_tx(client, 0x3618, vcm_data_low);
  ret|=subdev_i2c_tx(client, 0x3619, vcm_data_high);
...
//	struct v4l2_subdev *sdev = &act_ctrl->sdev;
  ...
  if(mode==1)
  {
  	...
  	ret=subdev_i2c_write(act_ctrl,0, 0);
  	if(ret==0)
