...
#define DEV_DBG_EN   		1
#if(DEV_DBG_EN == 1)		
#define csi_dev_dbg(x,arg...) pr_err("[CSI_DEBUG][SP0838]"x,##arg)
#else
#define csi_dev_dbg(x,arg...) 
#endif
#define csi_dev_err(x,arg...) pr_err("[CSI_ERR][SP0838]"x,##arg)
#define csi_dev_print(x,arg...) pr_err("[CSI][SP0838]"x,##arg)
...
{{0x22},{0xc0}},
{{0x26},{0x10}}, 
{{0x31},{0x10}},   //Upside/mirr/Pclk inv/sub //50
...
{{0x6b},{0x06}},
{{0x6c},{0x01}},
{{0x6d},{0x02}},  //Edge gain normal
{{0x6e},{0x42}},  //Edge gain normal
{{0x6f},{0x7f}},
{{0x70},{0x01}},
{{0x71},{0x09}},  //Èñ»¯ãÐÖµ           
{{0x72},{0x01}},  //ÈõÂÖÀªãÐÖµ         
{{0x73},{0x02}},  //±ßÔµÕýÏòÔöÒæÖµ     
{{0x74},{0x42}},  //±ßÔµ·´ÏòÔöÒæÖµ     
{{0x75},{0x7f}},  //Ê¹ÄÜÎ»             
...
{{0xf5},{0x80}},
{{0xf7},{0x78-0x08}},  //AE target
{{0xf8},{0x63-0x08}},  
{{0xf9},{0x68-0x08}},  //AE target
{{0xfa},{0x53-0x08}},
{{0xfd},{0x01}},  //P1
