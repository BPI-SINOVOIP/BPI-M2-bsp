...
static void gmac_get_power_io(void);
static int gmac_enable_power(void);
static int gmac_disable_power(void);
/**
 * gmac_get_power_io
 * Description:get gmac power io handler
 */
static void gmac_get_power_io(void)
{
	int ret;
	script_item_u val;
	ret = script_get_item("gmac_para", "gmac_power", &val);
	switch(ret)
	{
	case SCIRPT_ITEM_VALUE_TYPE_STR:
		//axp
		printk("it is a axp io,name %s\n",val.str);
		ldo = regulator_get(NULL, val.str);
		if(!ldo) {
			printk("an unknown axp io name %s\n", val.str);
			return;
		}
		//make only axp io actived
		break;
	case SCIRPT_ITEM_VALUE_TYPE_PIO:
		//get ic gpio here
		printk("it is a ic io");
		ldo = NULL; //make only gpio actived
		break;
	default:
		printk("neither a ic io nor axp io.");
		ldo = NULL;
		break;
	}
}
/**
 * gmac_enable_power
 * Description:enable gmac power.
 */
static int gmac_enable_power(void)
{
	//the axp io handler and ic io handler can not be actived at the same time
	int ret;
	gmac_get_power_io();
	printk("set gmac power");
	if(ldo)
	{
		printk("---1--");
		...
		if(ret < 0)
		{
			printk("set voltage for axp io fail\n");
			regulator_put(ldo);
			return -1;
		}
...
/**
 * gmac_disable_power
 * Description:disable gmac power.
 */
static int gmac_disable_power(void)
{
	int ret;
	gmac_get_power_io();
	printk("disable gmac power");
	if(ldo)
	{
		printk("---0--");
		...
		if(ret < 0)
		{
			printk("disable for axp io fail\n");
			regulator_put(ldo);
			return -1;
		}
		regulator_put(ldo);
...
int gmac_resume(struct net_device *ndev)
{
	...
	if (ndev->phydev)
		phy_start(ndev->phydev);
	/* At super standby mode, restart gmac */
	gmac_release(ndev);
	msleep(50);
	gmac_open(ndev);
	return 0;
...
extern struct gmac_plat_data gmac_platdata;
static void gmac_get_interface() {
	int ret;
	script_item_u val;
	ret = script_get_item("gmac_para", "gmac_interface", &val);
	if(SCIRPT_ITEM_VALUE_TYPE_INT != ret) {
		printk(KERN_WARNING "[sunxi_gmac]: Script is failed,use default interface mode\n");
		return;
	}
	if(val.val < 4 && val.val > 0) {
		printk("gmac_interface = %d\n",val.val);
		gmac_platdata.phy_interface = val.val;
		if( val.val == 3) {
				gmac_platdata.phy_interface = PHY_INTERFACE_MODE_RGMII;
		}
		else {
				gmac_platdata.phy_interface = val.val;
		}
		gmac_device.dev.platform_data = &gmac_platdata;
	} else {
		printk(KERN_WARNING "[sunxi_gmac]: invaled gmac_interface mode,use default interface mode\n");
	}
}
...
#endif
	printk("start to register gmac_device&gmac_driver\n");
	gmac_get_interface();
