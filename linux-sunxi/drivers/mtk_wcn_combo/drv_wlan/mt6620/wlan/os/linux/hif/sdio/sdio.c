    ...
    )
{
    ...
    if (bNum > 0) { // block mode
    	//printk("------%s,%d,pucSrc:0x%x,blk_sz:0x%x,bNum:0x%x----\n",__func__,__LINE__,pucSrc,((prGlueInfo->rHifInfo).prFuncInfo->blk_sz),bNum);
        ret = mtk_wcn_hif_sdio_write_buf(prGlueInfo->rHifInfo.cltCtx, u2Port, (PUINT32) pucSrc,
                             ((prGlueInfo->rHifInfo).prFuncInfo->blk_sz) * bNum);
    }
    else {  // byte mode
    	count = (count+3)/4*4;
    	//printk("------%s,%d,pucSrc:0x%x,count:0x%x----\n",__func__,__LINE__,pucSrc,count);
