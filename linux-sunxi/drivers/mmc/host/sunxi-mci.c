...
s32 sw_mci_set_vddio(struct sunxi_mmc_host* smc_host, u32 vdd)
{
	...
	//SMC_MSG(smc_host, "sdc%d try to switch io voltage,regulator\n", smc_host->pdev->id,smc_host->regulator);
	...
	switch (vdd) {
		case SDC_WOLTAGE_3V3:
			...
			regulator_set_voltage(smc_host->regulator, 3000000, 3000000);
			if (!on[id]) {
		...
		case SDC_WOLTAGE_OFF:
			if (on[id]) {
				...
				//regulator_force_disable(smc_host->regulator);			
					regulator_disable(smc_host->regulator);
					if (IS_ERR(smc_host->regulator)) {
						SMC_ERR(smc_host,"%s:regulator off Failed!\n",
								__FUNCTION__);
				    } else {
					    SMC_DBG(smc_host,"%s:regulator off sucessed!\n",
								__FUNCTION__);
			    	}		 
				on[id] = 0;
			}
...
s32 sw_mci_get_vddio(struct sunxi_mmc_host* smc_host)
{
	int voltage ;
	...
	switch (ios->power_mode) {
		...
		case MMC_POWER_UP:
			if (!smc_host->power_on) {
				...
				smc_host->power_on = 1;
				sw_mci_set_vddio(smc_host, smc_host->regulator_voltage);
...
static int sw_mci_do_voltage_switch(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (smc_host->voltage != SDC_WOLTAGE_3V3 &&
	    ...
	    /* if emmc use voltage 1.8V init ,not need switch */
		if (smc_host->cd_mode == CARD_ALWAYS_PRESENT ) {
			SMC_INFO(smc_host,"%s mmc %d return, not neet to switch voltage to 3v3: \n",
	...
	for (i=0; i<4; i++) {
		...
		if (type != SCIRPT_ITEM_VALUE_TYPE_STR) {
			...
			if (!strcmp(val.str, "none")) {
			    ...
			    SMC_MSG(NULL,"%s:card%d regulator id %s \n",__FUNCTION__,i,val.str);
				//mmcinfo->regulator = NULL;
				mmcinfo->caps &= ~ MMC_CAP_1_8V_DDR;
			}
		}
	}
