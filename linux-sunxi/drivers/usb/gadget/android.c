...
extern int get_chip_id(uint8_t chip_id[16]);
...
#else
{
...
#ifdef CONFIG_USB_SERIAL_FROM_AXP_CHIP_ID
	usb_config.serial_unique = 1;
#endif
	...
	if(usb_config.serial_unique){
		...
		sprintf(serial_string, "%02x%06x%03x%08x", chipid[0],chipid[1],chipid[2],chipid[3]);
	}else{
