...
// BPI-M2: Thanks for linux-sunxi support!!
...
//screen0
    ...
    // for av.
    if(OSAL_Script_FetchParser_Data("tv0_para", "tv_used", &value, 1) < 0)
    {
        __wrn("AV is unsupported in this device.\n");
    }
    else {
        init_para->av_support[0] = value;
    }
...
#else
    init_para->buffer_num[0]= 2; //video
printk("MIKEY: video init_para->buffer_num[0]=%d\n", init_para->buffer_num[0]);
#endif
...
int dispc_gralloc_queue(setup_dispc_data_t *psDispcData, int ui32DispcDataLength, void (*cb_fn)(void *, int), void *cb_arg)
{
    ...
    for(disp = 0; disp < 2; disp++)
    {
        if(!psDispcData->show_black[disp])
        {
            ...
            for(i=0; i<4; i++)
            {
                ...
                if(i < layer_num)
                {
			...
			#if 1
                   if(disp == 0 && BSP_disp_hdmi_get_mode(0) == DISP_TV_MOD_1080P_24HZ_3D_FP && layer_info.fb.mode == DISP_MOD_INTERLEAVED )
                   {
                        layer_info.fb.b_trd_src = 1;
                        layer_info.fb.trd_mode = DISP_3D_SRC_MODE_FP;
                        layer_info.b_trd_out = 1;
                        layer_info.out_trd_mode = DISP_3D_SRC_MODE_FP;
                        layer_info.scn_win.width = 1920;
                        layer_info.scn_win.height = 2160;

                        layer_info.fb.trd_right_addr[0] = layer_info.fb.addr[0] + layer_offset;
                   ...
                   //modify by linjunqian
                   if(BSP_disp_get_layer_enable(disp)){
                        BSP_disp_layer_open(disp, hdl);
                    }else{
...
{    
    ...
    if(g_fbi.disp_init.b_init)
    {
        ...
        if(g_fbi.disp_init.av_support[0])
        {
            BSP_disp_set_av_support_flag(0,g_fbi.disp_init.av_support[0]);
        }
