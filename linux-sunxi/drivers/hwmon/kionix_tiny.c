...
//#define dprintk(level_mask, fmt, arg...)	printk(KERN_ERR fmt, ## arg)
...
static int __devinit kionix_kj9_probe(struct i2c_client *client,
...
static int __devexit kionix_kj9_remove(struct i2c_client *client)
{
	int result;
...
static struct i2c_driver kionix_kj9_driver = {
	...
	.remove	= __devexit_p(kionix_kj9_remove),
#ifdef CONFIG_HAS_EARLYSUSPEND
#else
#ifdef CONFIG_PM
...
static int __init kionix_kj9_init(void)
{
	...
	kionix_kj9_driver.detect = gsensor_detect;
