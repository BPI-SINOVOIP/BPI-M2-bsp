...
int gpio_control(void)
{
	...
	printf("don't initialize the user_gpio\n");
	return 0;
}
...
************************************************************************************************************
*/
int check_usb_recovery_key(void)
{
	user_gpio_set_t     gpio_recovery;
	__u32			    gpio_hd;
	int					gpio_value;
	...
	ret = script_parser_fetch("system", "anrecovery_key", (int *)&gpio_recovery, sizeof(user_gpio_set_t)/4);
	if (!ret)
	{
		gpio_recovery.mul_sel = 0;		//强制设置成输入
		gpio_hd = gpio_request(&gpio_recovery, 1);
