...
void boot_power_data_trans(void)
{
...
__s32 boot_power_get_key(void)
{
    ...
    if(power_standby_pmu_type == PMU_TYPE_AXP209)
    {
        ...
        if(key_exist)
        {
        	...
        	i2c_write(AXP20_ADDR, reg_addr, 1, &value, 1);
    	}
    }
    else
    {
...
__s32 boot_power_battery_charge_status(void)
{
    ...
    if(power_standby_pmu_type == PMU_TYPE_AXP209)
    {
        ...
        if(charge_status)
        {
        	...
        	i2c_read(AXP20_ADDR, reg_addr, 1, &value, 1);
	    }
...
__s32 boot_power_axp_rest_cal(void)
{
	...
	if(power_standby_pmu_type == PMU_TYPE_AXP209)
	{
    	...
    	if(i2c_read(AXP20_ADDR, reg_addr, 1, &value1, 1))
    	{
        	return -1;
    	}
    	...
    	if(i2c_read(AXP20_ADDR, reg_addr, 1, &value2, 1))
    	{
        	return -1;
    	}
  		...
  		if(i2c_write(AXP20_ADDR, reg_addr, 1, &value1, 1))
        {
            return -1;
        }
	}
...
__s32 boot_power_int_enable(void)
{
    ...
    if(i2c_read(AXP20_ADDR, reg_addr, 1, power_int_value, 5))
    {
        return -1;
    }
	...
	for(i=0;i<5;i++)
	{
    	...
    	if(i2c_write(AXP20_ADDR, reg_addr, 1, int_enable + i, 1))
    	{
        	return -1;
    	}
    }
...
__s32 boot_power_int_disable(void)
{
	...
	for(i=0;i<5;i++)
	{
    	...
    	if(i2c_write(AXP20_ADDR, reg_addr, 1, power_int_value + i, 1))
    	{
        	return -1;
    	}
    }
...
//    }
	for(i=0;i<5;i++)
	{
    	...
    	if(i2c_read(AXP20_ADDR, reg_addr, 1, int_status + i, 1))
    	{
        	return -1;
    	}
        /*if(BOOT_TWI_Write(AXP20_ADDR, &reg_addr, int_status + i))*/
    	if(i2c_write(AXP20_ADDR, reg_addr, 1, int_status + i, 1))
    	{
        	return -1;
    	}
    }
